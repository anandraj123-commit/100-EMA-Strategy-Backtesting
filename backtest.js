...
function runBacktest(candles, p) {
  ...
  for (let i = warmup; i < candles.length; i++) {
    ...
    } else if (dayBlocked && (buySignalCandle || sellSignalCandle)) {
      tradesSkippedDueToLimit++;
    }

    if (!dayBlocked && pendingBuyTrigger != null && c.high > pendingBuyTrigger) {
      ...
      if (stopPct < p.minStopPct) {
        ...
        activeTrades.push({
          tradeDir: 'long', entryPrice: candidateEntry, slPrice: candidateSL,
          tpPrice: candidateTP, entryTime: c.time, riskAmount,
          positionQty: riskBasedQty
        });
      }

      pendingBuyTrigger = null; pendingBuySL = null; pendingBuySignalIndex = null;
    } else if (!dayBlocked && pendingSellTrigger != null && c.low < pendingSellTrigger) {
      ...
      if (stopPct < p.minStopPct) {
        ...
        activeTrades.push({
          tradeDir: 'short', entryPrice: candidateEntry, slPrice: candidateSL,
          tpPrice: candidateTP, entryTime: c.time, riskAmount,
          positionQty: riskBasedQty
        });
      }
    ...
    for (let tradeIndex = activeTrades.length - 1; tradeIndex >= 0; tradeIndex--) {
      const trade = activeTrades[tradeIndex];
      const { tradeDir, entryPrice, slPrice, tpPrice, entryTime, riskAmount, positionQty } = trade;
